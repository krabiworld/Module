...
@Command(
	...
	args = "command.kick.args",
	help = "command.kick.help",
...
public class KickCommand extends AbstractCommand {
    ...
    protected void execute(CommandContext ctx) {
		...
		if (!ctx.getSelfMember().canInteract(member)) {
			ctx.sendError("command.kick.error.role.position");
			return;
		}
		if (member == ctx.getMember()) {
			ctx.sendError("command.kick.error.cannot.yourself");
			return;
		}

        ctx.getGuild().kick(member, reason).queue();
		ctx.sendSuccess("command.kick.success.kicked",
